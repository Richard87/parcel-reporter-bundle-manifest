    ...
    async report ({ event: event , options: options  }) {
         ...
         */ const asset = (ref = bundle.getMainEntry()) !== null && ref !== void 0 ? ref : bundle.getEntryAssets()[0];
                if (asset) {
                    ...
                    const assetName = $fd2c454dbd295085$var$normalisePath($parcel$interopDefault($5J5yU$path).relative(entryRoot, assetPath));
                    const bundleUrl = bundle.target.publicUrl + "/" + $fd2c454dbd295085$var$normalisePath($parcel$interopDefault($5J5yU$path).relative(bundle.target.distDir, bundle.filePath));
                    manifest[assetName] = bundleUrl;
                }
            }
            const targetPath = `${targetDir}/${$fd2c454dbd295085$export$942f2ec118ed46e4}`;
            await options.outputFS.writeFile(targetPath, JSON.stringify(manifest, null, 2));
